...
class TestOrderingService:
    def test_raises_when_order_already_created(self, ordering):
        ...
        order_id = ordering.create_buy_order(command)

        with raises(OrderAlreadyExists) as exists:
            ordering.create_buy_order(command)

        assert exists.value.order_id == order_id
    ...
    def test_emmit_event_when_buy_order_created(self, ordering):
        cmd_id = uuid4()
        order_id = ordering.create_buy_order(command_id=cmd_id)
        ordering.expect(BuyOrderCreated, command_id=cmd_id, order_id=order_id)
...
class OrderingSteps:
    ...
    def _service(self) -> Service:
        return self._container.create_object(
            ...
            additional_kwargs={
                ...
                "settings": self._container.create_object(
                    Settings,
                    additional_kwargs={"ordered_btc_limit": self._btc_limit},
                )
            }
        )
    ...
    def create_buy_order(
        ...
        return self._service.create_buy_order(
